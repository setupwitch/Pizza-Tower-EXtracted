...
with (instance_create_depth(0, 0, depth - 1, obj_itemlist))
{
	...
	{
		var a = 1;
		if (_is_transparent > 0)
		{
			a = 0.5;
		}
		draw_sprite_ext(spr_emptybg, 0, _x, _y, sprite_width / 64, item_height / 64, 0, c_white, a);
		var scale = sprite_width / sprite_get_width(_item);
		var sx = 0;
		var bb_top = sprite_get_bbox_top(_item);
		var bb_bottom = sprite_get_bbox_bottom(_item);
		var sy = bb_top + ((bb_bottom - bb_top) / 4);
		draw_sprite_part_ext(_item, 0, sx, sy, sprite_width / scale, item_height / scale, _x, _y, scale, scale, c_white, a);
	};
}

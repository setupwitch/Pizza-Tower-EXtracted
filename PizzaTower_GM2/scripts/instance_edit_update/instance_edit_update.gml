function object_apply_values(_obj)
{
	sprite_index = asset_get_index(_obj.sprite_index);
	snap_tile_val = _obj.snap_tile_val;
	xoffset = _obj.xoffset;
	yoffset = _obj.yoffset;
}
	...
	switch (instance_state)
	{
					...
					else
					{
						...
						with (instance_place(xx, yy, obj_editorobject))
						{
							...
							with (other)
							{
								...
								instance_state = instancestates.move;
		...
		case instancestates.move:
			...
			if (key_place_released)
			{
				...
				instance_state = instancestates.normal;
			}
			break;
	}
}

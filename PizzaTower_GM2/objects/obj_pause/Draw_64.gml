...
if (fade > 0 && lvlsave != noone && lvlsave != "tutorial" && lvlsave != "secretworld" && lvlsave != "exit" && room != boss_fakepep && room != boss_fakepephallway && room != boss_fakepepkey && room != boss_vigilante && room != boss_noise && room != boss_pepperman && room != boss_pizzaface && room != Endingroom && room != Johnresurrectionroom && room != Creditsroom)
{
	if (!instance_exists(obj_achievement_pause))
	{
...
if (transfotext != noone && !instance_exists(obj_achievement_pause))
{
	draw_set_alpha(fade);

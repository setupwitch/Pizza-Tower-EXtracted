	...
	if (((key_jump || key_back) && key_select <= -1) || (key_select > -1 && key_back))
	{
		...
		for (var i = 0; i < array_length(input); i++)
		{
			...
			if (_v != "player_superjump" && _v != "player_groundpound")
			{
				...
				if (!is_undefined(tdp_input_get(_v)) && tdp_input_get(_v).actions != noone && array_length(tdp_input_get(_v).actions) <= 0)
				{
					_found = true;
				}
			}
	...
	if (keyboard_check_pressed(vk_anykey))
	{
		if (keyboard_key != vk_f1)
		{
			if (!menu || scr_check_menu_repeats(input[key_select][0], keyboard_key, false))
			{
				...
				if (!in.has_value(tdp_input_actiontypes.keyboard, keyboard_key))
				{
					array_push(in.actions, new tdp_input_action(tdp_input_actiontypes.keyboard, keyboard_key));
				}
				selecting = false;
			}
...
{
	if (IS_WINDOWS && keyboard_check_pressed(vk_anykey))
	{
		startbuffer = 5;
		addbuffer = 5;
		selecting = false;
	...
	if (val != noone && val != gp_select)
	{
		if (!menu || scr_check_menu_repeats(input[key_select][0], val, true))
		{
			startbuffer = 5;
			addbuffer = 5;
			var in = tdp_input_get(concat(input[key_select][0], "C"));
			if (!is_array(val))
			{
				if (!in.has_value(tdp_input_actiontypes.gamepad_button, val))
				{
					array_push(in.actions, new tdp_input_action(tdp_input_actiontypes.gamepad_button, val));
				}
			}
			else if (!in.has_value(tdp_input_actiontypes.gamepad_axis, val[0], val[1]))
			{
				array_push(in.actions, new tdp_input_action(tdp_input_actiontypes.gamepad_axis, val[0], val[1]));
			}
			selecting = false;
		}
